    ...
    <div class="form-wrap">
      <div class="form-box">
        ...
        <div class="form-row">
          <div class="form-item">
            <span class="label">
              <i class="require"></i>选择会议室：
            </span>
            <Select
              class="form-select"
              v-model="formData.room_guid"
              label-in-value
              @on-change="handleSelectRoom"
            >
              <Option
                v-for="item in meetingRoomList"
                :value="item.room_guid"
                :key="item.room_guid"
        ...
        <div class="form-row">
          <div class="form-item">
            <span class="label">
              <i class="require"></i>会议时间：
            </span>
            <DatePicker
              v-model="formData.stime"
              @on-ok="handlePickDatetime"
              type="datetime"
              placeholder="开始时间"
  ...
  methods: {
    ...
    checkPickDateTime() {
      ...
      if (this.formData.stime) {
        stimeStamps = Math.floor(
          new Date(this.formData.stime).getTime() / 1000
        );
      }
      if (this.formData.etime) {
        etimeStamps = Math.floor(
          new Date(this.formData.etime).getTime() / 1000
        );
      }
