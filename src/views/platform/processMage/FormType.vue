<template>
  <div class="formType">
    <h4 class="formType-title">表单类型</h4>
    <Row class="formType-addBtn" >
  ...
  methods: {
    ...
    async handleType(){
      ...
      if(this.type == 'add'){
        ...
        res = await this.$http.addFormtype(this.addForm)
      }else {
        res = await this.$http.editFormtype(this.addForm)
    ...
    // 删除
    delFormtype(type_guid){
      this.$http.delFormtype({type_guid}).then( res => {
        if(res.code == 200){
          this.getFormtype();
          this.$Message.success(res.message);
        }else{  
    ...
    remove(item) {
      this.$Modal.confirm({
          title: '是否删除？',
          onOk: () => {
              this.delFormtype(item.type_guid);
          },
          onCancel: () => {
